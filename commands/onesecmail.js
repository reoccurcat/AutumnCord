...
module.exports = {
	...
	async execute(interaction) {
        if (interaction.options.getSubcommand() === 'generate') {
            ...
            if (amnt > 1) {
                await interaction.reply({ content: `viewing emails for inbox \`${string}\``, embeds: [embed], components: [row] });
                ...
                collector.on('collect', async i => {
                    await i.deferUpdate()
                    if (i.customId === String(customId3)) {await interaction.editReply({components: [disabledRow]})}
                    else if (i.customId === String(customId1)) {page += 1}
                    else if (i.customId === String(customId2)) {page -= 1}
                    if (page < 1) {page = amnt} else if (page > amnt) {page = 1}
                    let date = test[parseInt(page - 1)].date + " +0100"
                    date = parseInt((new Date(date).getTime()/1000).toFixed(0))
                    let embed = new MessageEmbed()
                        .setColor('#5a1da1')
                        .setAuthor({name:`1SecMail (${page}/${amnt})`, iconURL:'https://autumncord.xyz/assets/images/mail.png'})
            ...
            } else if (amnt === 1) {await interaction.reply({content: `viewing emails for inbox \`${string}\``, embeds: [embed]})}
            else {await interaction.reply(`No emails have been sent to \`${string}\` yet.`)}
            ...
            date = parseInt((new Date(date).getTime()/1000).toFixed(0))
            let body
            if (test.textBody !== "\n") {body = test.textBody} else {body = "email has no content"}
            let embed = new MessageEmbed()
                .setColor('#5a1da1')
                .setAuthor({name:`1SecMail`, iconURL:'https://autumncord.xyz/assets/images/mail.png'})
